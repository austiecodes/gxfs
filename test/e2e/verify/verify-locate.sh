...
curl_put "$(repo_url "$REPO1" write "path=/docs/error-handling.md")" \
...
curl_get "$(repo_url "$REPO1" locate "q=error")"
...
# Locate across repo2
curl_get "$(repo_url "$REPO2" locate "q=error")"
...
curl_get "$(repo_url "$REPO1" locate "q=error&limit=1")"
RESULT_COUNT=$(echo "$BODY" | python3 -c "import sys,json; print(len(json.load(sys.stdin).get('results',[])))" 2>/dev/null || echo "0")
...
if [ -n "$RESULT_PATH" ]; then
    curl_get "$(repo_url "$REPO1" cat "path=${RESULT_PATH}")"
...
curl_get "$(repo_url "$REPO1" locate "q=xyznonexistentquery")"
...
curl_get "$(repo_url "$REPO1" stat "path=/docs/testing.md")"

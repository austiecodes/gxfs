...
func TestAuditPathLocalGxfsDir(t *testing.T) {
	tmpDir := t.TempDir()
	origDir, _ := os.Getwd()
	...
	if err := os.MkdirAll(".gxfs", 0o755); err != nil {
		t.Fatal(err)
	}

	got := auditPath()
	want := filepath.Join(".gxfs", "audit.jsonl")
	if got != want {
...
func TestAuditPathGlobalFallback(t *testing.T) {
	...
	want := filepath.Join(home, ".gxfs", "audit.jsonl")
	if got != want {
		t.Errorf("auditPath() = %q, want %q", got, want)
	}
}

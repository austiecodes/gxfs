...
func TestPostgresConfigFromServerDefaultsFileTable(t *testing.T) {
	...
	if cfg.DSN != "postgres://localhost/rolio" || cfg.Schema != "public" {
...
`)
	...
	if cfg.Backend.Postgres.DSN != "postgres://localhost/rolio" ||
...
func TestStorageTargetLabel(t *testing.T) {
	target := storageTarget{
		dsn:    "postgres://secret:password@prod-db:5432/rolio",
		schema: "tenant1",
	}

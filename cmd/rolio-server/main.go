...
func newRootCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "rolio-server",
		Short: "ROLIO virtual filesystem server",
		Long:  "ROLIO server provides a REST API for virtual filesystem content backed by PostgreSQL.",
...
Orphan documents are those that:
  - Have no references in rolio_repo_paths
  - Have no references in rolio_docset_docs
...
Use --force to actually delete.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			path := configPath
			if path == "" {
				path = os.Getenv("ROLIO_SERVER_CONFIG")
			}
			if path == "" {
	...
	cmd.Flags().StringVarP(&configPath, "config", "c", "", "Path to server config file (default: $ROLIO_SERVER_CONFIG or conf/server.toml)")

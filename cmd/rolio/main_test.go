...
import (
	...
	"github.com/austiecodes/rolio/internal/mount"
	"github.com/austiecodes/rolio/internal/server"
	"github.com/austiecodes/rolio/internal/store"
	"github.com/austiecodes/rolio/internal/syncmanifest"
...
func (f *sourceRoutingFakeClient) AdapterForSource(_ context.Context, source store.SourceRef) (store.Adapter, error) {
	...
	case store.SourceKindRepo:
		if source.Name == "rolio" || source.Name == "self" {
...
func execute(t *testing.T, args ...string) (string, *fakeClient) {
	...
	client := &fakeClient{}
	cmd := newRootCommand(client, client, "rolio", nil)
...
func TestRunHelpDoesNotRequireConfig(t *testing.T) {
	...
	if !strings.Contains(stdout.String(), "ROLIO gives agents Unix-like commands") {
...
func TestRunInitDoesNotRequireConfig(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, ".rolio", "settings.toml")); err != nil {
		t.Fatalf("settings.toml stat error = %v", err)
	}
	if _, err := os.Stat(filepath.Join(dir, ".rolio", "mounts.toml")); err != nil {
...
func TestRunUsesMountsForRemotePathResolution(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Query().Get("repo") != "rolio" {
			t.Fatalf("request repo = %q, want rolio", r.URL.Query().Get("repo"))
		}
...
path = "docs"
`
	if err := os.WriteFile(filepath.Join(rolioDir, "settings.toml"), []byte(settings), 0o644); err != nil {
...
source = "default"
`
	if err := os.WriteFile(filepath.Join(rolioDir, "mounts.toml"), []byte(mounts), 0o644); err != nil {
		t.Fatalf("write mounts.toml: %v", err)
	}

	t.Setenv("ROLIO_CONFIG", filepath.Join(rolioDir, "settings.toml"))
	t.Chdir(dir)
...
func TestFindRequiresNameOrIName(t *testing.T) {
	c := &fakeClient{findNodes: findMixedNodes()}
	cmd := newRootCommand(c, c, "rolio", nil)
	var out bytes.Buffer
	cmd.SetOut(&out)
	cmd.SetErr(&out)
...
func TestSyncPushUploadsFilesAndWritesManifest(t *testing.T) {
	...
	t.Chdir(dir)
	manifestPath := filepath.Join(dir, ".rolio", "manifest.toml")
	client := &fakeClient{}
...
func readROLIOSkillBundle(t *testing.T, home string) string {
	t.Helper()

	skillDir := filepath.Join(home, ".claude", "skills", "rolio-skill")
...
func TestInitWritesSettingsAgentsInstructionsAndSkill(t *testing.T) {
	...
	settings := readTextFile(t, filepath.Join(dir, ".rolio", "settings.toml"))
	...
	mounts := readTextFile(t, filepath.Join(dir, ".rolio", "mounts.toml"))
	...
	if !strings.Contains(agents, command.ROLIOInstructionsStart) || !strings.Contains(agents, command.ROLIOInstructionsEnd) {
		t.Fatalf("AGENTS.md missing ROLIO markers: %q", agents)
	}
	if !strings.Contains(agents, "rolio tree docs -L 3") || !strings.Contains(agents, "~/.claude/skills/rolio-skill/SKILL.md") {
		t.Fatalf("AGENTS.md missing instruction content: %q", agents)
	}
	if strings.Contains(agents, "rolio mount add") || strings.Contains(agents, "rolio sync") || strings.Contains(agents, "rolio write") {
		t.Fatalf("AGENTS.md should stay minimal, got: %q", agents)
	}
	if !strings.Contains(got, "updated ROLIO instructions in") {
		t.Fatalf("init output = %q, want instruction update", got)
	}
	if !strings.Contains(got, "updated ROLIO skill in") {
		t.Fatalf("init output = %q, want skill update", got)
	}
	skill := readTextFile(t, filepath.Join(home, ".claude", "skills", "rolio-skill", "SKILL.md"))
	...
	if _, err := os.Stat(filepath.Join(home, ".claude", "skills", "rolio-skill", "references", "browse.md")); err != nil {
...
func TestInitModeSkillWritesSkillOnly(t *testing.T) {
	...
	skillPath := filepath.Join(home, ".claude", "skills", "rolio-skill", "SKILL.md")
	skill := readTextFile(t, skillPath)
	bundle := readROLIOSkillBundle(t, home)
	if !strings.Contains(skill, `references/discovery.md`) || !strings.Contains(skill, `references/setup-hooks-ops.md`) {
		t.Fatalf("SKILL.md missing reference index: %q", skill)
	}
	for _, want := range []string{
		`rolio ls knowledge`,
		`rolio cat knowledge/foo.md`,
		`rolio grep "pattern" knowledge`,
		`rolio find knowledge --name "*.md"`,
		`rolio stat knowledge/foo.md`,
		`rolio rm knowledge/new.md`,
		`rolio mount sources`,
		`docs://openai-go-sdk/reference`,
		`rolio sync refresh knowledge`,
		`rolio sync materialize knowledge`,
		`rolio sync dematerialize knowledge --keep-files`,
	} {
...
func TestInitModeMdSkillWritesMarkdownAndSkill(t *testing.T) {
	...
	agents := readTextFile(t, filepath.Join(dir, "AGENTS.md"))
	if !strings.Contains(agents, command.ROLIOInstructionsStart) {
		t.Fatalf("AGENTS.md missing ROLIO instructions: %q", agents)
	}
	bundle := readROLIOSkillBundle(t, home)
	if !strings.Contains(bundle, `rolio mount sources`) || !strings.Contains(bundle, `docs://openai-go-sdk/reference`) {
		t.Fatalf("generated skill bundle missing ROLIO content: %q", bundle)
	}
	if !strings.Contains(got, "updated ROLIO instructions in") || !strings.Contains(got, "updated ROLIO skill in") {
...
func TestInitModeMdWritesMarkdownOnly(t *testing.T) {
	...
	agents := readTextFile(t, filepath.Join(dir, "AGENTS.md"))
	if !strings.Contains(agents, "~/.claude/skills/rolio-skill/SKILL.md") {
	...
	if _, err := os.Stat(filepath.Join(home, ".claude", "skills", "rolio-skill", "SKILL.md")); !os.IsNotExist(err) {
...
func TestInitRejectsUnsupportedMode(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, ".rolio", "settings.toml")); !os.IsNotExist(err) {
...
func TestInitReplacesExistingInstructions(t *testing.T) {
	...
	agents := readTextFile(t, filepath.Join(dir, "AGENTS.md"))
	if got := strings.Count(agents, command.ROLIOInstructionsStart); got != 1 {
		t.Fatalf("ROLIO start marker count = %d, want 1 in %q", got, agents)
	}
	if got := strings.Count(agents, command.ROLIOInstructionsEnd); got != 1 {
		t.Fatalf("ROLIO end marker count = %d, want 1 in %q", got, agents)
	}
}
...
func TestInitAgentClaudeWritesClaudeMD(t *testing.T) {
	...
	if !strings.Contains(claude, command.ROLIOInstructionsStart) {
		t.Fatalf("CLAUDE.md missing ROLIO instructions: %q", claude)
	}
	if _, err := os.Stat(filepath.Join(dir, "AGENTS.md")); !os.IsNotExist(err) {
...
func TestInitWritesSettingsAndMountsTemplatesFromFlags(t *testing.T) {
	...
	executeInit(t,
		...
		"--server", "https://rolio.example.com",
		"--docs", "knowledge",
		"--auth", "bearer",
		dir,
	)
...
func TestInitRegisterExistingRepoIsIdempotent(t *testing.T) {
	...
	if !strings.Contains(got, "initialized "+filepath.Join(dir, ".rolio")) {
...
func TestRunRegisteredSlashRepoUsesCanonicalRepoInRequests(t *testing.T) {
	...
	t.Setenv("ROLIO_CONFIG", filepath.Join(dir, ".rolio", "settings.toml"))
	t.Chdir(dir)
	runOK("ls", "docs")
	runOK("tree", "docs")
...
func TestInitRejectsInvalidRepoNamesBeforeWriting(t *testing.T) {
	...
	for _, repo := range tests {
		t.Run(fmt.Sprintf("%q", repo), func(t *testing.T) {
			...
			if _, statErr := os.Stat(filepath.Join(dir, ".rolio", "settings.toml")); !os.IsNotExist(statErr) {
...
func TestInitReportsResolvedSymlinkTarget(t *testing.T) {
	...
	if !strings.Contains(claude, command.ROLIOInstructionsStart) {
		t.Fatalf("CLAUDE.md missing ROLIO instructions through symlink: %q", claude)
	}
...
func TestMountAddWritesReadonlyDocsetSourceMount(t *testing.T) {
	...
	cmd := newRootCommand(router, router, "rolio", nil)
	var out bytes.Buffer
	cmd.SetOut(&out)
	cmd.SetErr(&out)
...
func TestMountAddWritesMountsToml(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join(dir, ".rolio", "mounts.toml"))
	if err != nil {
...
func TestMountRemoveBlocksOnMaterialized(t *testing.T) {
	...
	manifestDir := filepath.Join(dir, ".rolio")
...
func TestMountSourcesJSONOutput(t *testing.T) {
	...
	if len(resp.Sources) != 1 || resp.Sources[0].Ref != "repo://rolio" || resp.Sources[0].Kind != store.SourceKindRepo {
		t.Fatalf("mount sources JSON = %+v, want rolio repo source", resp)
	}
}
...
func TestRefreshMountedRemoteDocCorrect(t *testing.T) {
	...
	resolver, err := mount.NewResolver("rolio", []config.MountConfig{
	...
	cmd := newRootCommand(client, client, "rolio", resolver)
	var out bytes.Buffer
	cmd.SetOut(&out)
	cmd.SetErr(&out)
	...
	manifest, err := syncmanifest.Load(filepath.Join(".rolio", "manifest.toml"))
	if err != nil {
...
func TestRefreshDocsMountedRemoteDocPreservesDocsSource(t *testing.T) {
	...
	cmd := newRootCommand(adapter, router, "rolio", resolver)
	var out bytes.Buffer
	cmd.SetOut(&out)
	cmd.SetErr(&out)
...
func TestHookSessionStartNoConfig(t *testing.T) {
	...
	// Should not error even without .rolio config.
	if err := cmd.Execute(); err != nil {
...
func TestHookSessionStartNoMounts(t *testing.T) {
	...
	// Create .rolio/settings.toml and empty mounts.toml
	rolioDir := filepath.Join(dir, ".rolio")
	if err := os.MkdirAll(rolioDir, 0o755); err != nil {
		t.Fatal(err)
	}
	writeTestFile(t, filepath.Join(rolioDir, "settings.toml"), "repo = \"test\"\n[server]\naddr = \"http://localhost:7635\"\n")
	writeTestFile(t, filepath.Join(rolioDir, "mounts.toml"), "version = 1\nmounts = []\n")
...
func TestClaudeHooksCreatesSettings(t *testing.T) {
	...
	if !strings.Contains(cmd, "rolio") {
		t.Errorf("hook command = %q, want rolio path", cmd)
	}
}
...
func TestHookSessionStartOverwritesUnchangedLocal(t *testing.T) {
	...
	// Setup: .rolio dir with settings, mounts, manifest, and a materialized file.
	rolioDir := filepath.Join(dir, ".rolio")
	os.MkdirAll(rolioDir, 0o755)
	writeTestFile(t, filepath.Join(rolioDir, "settings.toml"), "repo = \"test\"\n[server]\naddr = \"http://localhost:7635\"\n")
	...
	if err := syncmanifest.Save(filepath.Join(rolioDir, "manifest.toml"), manifest); err != nil {
		t.Fatal(err)
	}
	...
	updated, err := syncmanifest.Load(filepath.Join(rolioDir, "manifest.toml"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestStatErrorNotTreatedAsCreateOnly(t *testing.T) {
	...
	if err := config.SaveMounts(filepath.Join(dir, ".rolio", "mounts.toml"), mounts); err != nil {
		t.Fatal(err)
	}

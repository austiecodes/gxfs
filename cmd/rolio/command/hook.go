...
func newHookSessionStartCommand(adapter, rawAdapter store.Adapter, repo string, resolver *mountadapter.Resolver) *cobra.Command {
	return &cobra.Command{
		Use:   "session-start",
		Short: "Refresh rolio docs for a new agent session",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				fmt.Fprintf(cmd.OutOrStdout(), "rolio hook: %s\n", err)
				return nil
			}
			if updated > 0 {
				fmt.Fprintf(cmd.OutOrStdout(), "rolio: updated %d file%s\n", updated, plural(updated))
			}
			return nil
		},
	}
	...
	for _, root := range roots {
		...
		for i := range entries {
			...
			if entry.Materialized && entry.ContentHash != rf.ContentHash {
				...
				fmt.Fprintf(w, "rolio: skip %s: local has unpushed changes\n", entry.Local)
				updatedEntries = append(updatedEntries, entry)
				continue
			}
			updatedEntries = append(updatedEntries, newEntry)

		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if crossRepo {
				...
				if hasBaseline {
					...
					if statErr == nil {
						return fmt.Errorf("path %s exists remotely but has no local baseline; run 'rolio sync refresh' or 'rolio sync pull' first", path)
					}
			...
			if err != nil {
				if errors.Is(err, store.ErrConflict) {
					return fmt.Errorf("%w\nhint: run 'rolio sync pull' or 'rolio sync refresh' to update local state, then retry", err)
				}
				return err
			}
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if isCrossRepoMount(path, repo, resolver) {
				hash, hasBaseline := manifestHashForPath(path)
				if !hasBaseline {
					return fmt.Errorf("path %s has no local baseline; run 'rolio sync refresh' or 'rolio sync pull' first", path)
				}
				expectedHash = hash
			}
			setMountPathOnClient(rawAdapter, path, resolver)

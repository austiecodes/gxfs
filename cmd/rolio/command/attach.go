...
Examples:
  rolio mount attach openai-go --into docs/lib/openai-go
  rolio mount attach github/openai-go --into docs/lib/openai-go
  rolio mount attach openai --into docs/lib/openai-go --dry-run`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			switch len(matches) {
			case 0:
				return fmt.Errorf("no repos matched %q. Use 'rolio repo ls' to see available repos", keyword)
			case 1:
			...
			default:
				...
				fmt.Fprintf(cmd.OutOrStdout(), "Use: rolio mount attach <exact-repo-name> --into %s\n", localPath)

		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if manifest, err := syncmanifest.Load(manifestPath); err == nil {
				...
				for _, e := range entries {
					if e.Materialized {
						return fmt.Errorf("cannot remove mount %s: materialized files exist under this path (run `rolio sync dematerialize %s` first)", localPath, localPath)
					}
				}
			}
...
func defaultMountsPath() string {
	return filepath.Join(".rolio", "mounts.toml")
}
